...
| --- | --- | --- | --- | --- | --- | --- |
| 01 | discovery: team list + team pages → contest_ids | `python/ncaa_mfb_01_schedules_scrape.py` | `scripts/run_01_schedules.sh` | online (≈1 page/team) | persisted `mfb/teams/html/`, `mfb/schedules/html/{ay}/` re-read, not re-fetched — **completed seasons only**. An in-progress season needs `--refresh-discovery` (team pages only link played games; the daily driver passes it) | `NCAA_VENDOR=decodo_patchright ./scripts/run_01_schedules.sh --academic-year 2026 --division 11` |
